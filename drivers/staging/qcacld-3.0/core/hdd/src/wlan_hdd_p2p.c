			...
			QDF_STATUS status, uint32_t scan_id)
{
	...
	queue_delayed_work(system_freezable_power_efficient_wq, &hdd_ctx->roc_req_work, 0);
	...
	*cookie = (u64)roc_id;
	...
	if (REMAIN_ON_CHANNEL_REQUEST == request_type) {
		...
		if ((NULL != sta_adapter) &&
			...
			queue_delayed_work(system_freezable_power_efficient_wq, &pHddCtx->roc_req_work,
			msecs_to_jiffies(
				pHddCtx->config->p2p_listen_defer_interval));
	...
	if (isBusy == false && pAdapter->is_roc_inprogress == false) {
		...
		queue_delayed_work(system_freezable_power_efficient_wq, &pHddCtx->roc_req_work, 0);
	}
	return 0;
}
